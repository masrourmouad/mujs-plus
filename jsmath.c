...
{
	int min = js_isdefined(J, 1) ? js_tointeger(J, 1) : 0;
	int max = js_isdefined(J, 2) ? js_tointeger(J, 2) : 0;
	
	if(min < 0 || max < 0)                   js_error(J, "min and max must be a positive integer");
	if(js_tonumber(J, 1) != min)             js_error(J, "min must be a positive integer");
	if(max != 0 && js_tonumber(J, 2) != max) js_error(J, "max must be a positive integer");
	if(min > max && js_isdefined(J, 2))      js_error(J, "max should be bigger than min");
